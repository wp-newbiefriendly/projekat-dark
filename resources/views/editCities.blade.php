...
@section('content')
    ...
    <form method="POST" action="{{ route('updateCities', ['city' => $city->id]) }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="city" class="form-control" value="{{ old('name', $city->city) }}" placeholder="Naziv" required>
        </div>

        <div class="mb-3">
            <label for="temperatures" class="form-label">Opis</label>
            <textarea name="temperatures" class="form-control" placeholder="Temperatura">{{ old('description', $city->temperatures) }}</textarea>
        </div>
